...
export default function Dashboard({
  ...
  const triggerSimulation = propTriggerSimulation || (() => {
    ...
    setTimeout(() => {
      setLocalInjectionLog('Hooking Luau thread scheduler...');
    }, 700);
  ...
  useEffect(() => {
    if (injectionStatus === 'SUCCESS') {
      setLogs(prev => {
        if (prev.some(log => log.includes('Luau executor attached successfully'))) return prev;
        return [
          ...prev,
  ...
  return (
    ...
    >
      ...
      <div className="p-6 space-y-8 max-w-7xl mx-auto w-full z-10 relative text-left pt-10">
        ...
        {/* Sleek, creative, and non-corny welcome header row with status in top right */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 pb-4 border-b border-zinc-800/30">
          <div className="flex flex-col space-y-1.5 text-left">
            <span className="text-[10px] font-mono tracking-widest text-zinc-500 uppercase font-extrabold">Luau Execution Environment</span>
